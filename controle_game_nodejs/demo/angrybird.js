/**
 * Angry Birds Nokia
 * Produced by Wunderman Brazil - Sao Paulo
 * Code by Andre Mattos (http://www.ma77os.com/) and Felipe Pupo (http://conheca.me/)
 * Thanks to http://erkie.github.com/ - http://mrdoob.com/projects/chromeexperiments/google_gravity/ - http://box2d-js.sourceforge.net/
 *
 */
 
 // const
var SHARE_URL = "http://nokialat.am/pet5TN";
var FACEBOOK_TITLE = "¡Potenciá tu experiencia!";
var FACEBOOK_DESC = "¡Potenciá tu experiencia con un Nokia N8 y hacé pedazos cualquier sitio web! ¡yo ya hice [score] puntos!";
var TWITTER_TITLE = "¡Potenciá tu experiencia con un Nokia N8 y hacé pedazos cualquier sitio web! ¡yo ya hice [score] puntos! - [url]";
...
var HEIGHT;
var PAGE_HEIGHT;
//var BASE_URL = "http://127.0.0.1:8080/angrybirds/";
//var BASE_URL = "http://www.ma77os.com/angrybirds/";
var BASE_URL = "http://hml.nokia-jogoangrybirds.wunderman.com.br/angrybirds/";
var IMGS_URL = BASE_URL + "images/";
...
var TWO_PI = Math.PI*2;
 ...
 /* Based on Alex Arnell's inheritance implementation. */
    ...
    function AngryBirds()
	{
		// CLASSES
		/**
		 * SpriteSheet - adapted from "Animation" from the game Infector
		 * http://www.pluginmedia.net/dev/infector/ by Plugin Media
		 */
		function SpriteSheet(img, w, h, element, createElement) 
		{
			...
			if (!element && createElement)
			{
				...
				this.element.style.background = 'url('+this.img.src+')'; 
				...
				this.element.style.zIndex = 9999 + boxes.length;
			...
			this.updateObj = function() {
				...
				if (this.element)
				{
					this.element.style.left = Math.round(this.x+this.offsetX) + "px";
					this.element.style.top = Math.round(this.y+this.offsetY) + "px";
					this.element.style.background = 'url('+this.img.src+')'; 
					this.element.style.width = this.width+"px"; 
					this.element.style.height = this.height+"px";
		...
		/**
		 * Scene3D - a lightweight "false 3D" engine
		 */
		function Scene3D (focalLength, vanishPoint)
		{
			this.focalLength = focalLength || 250;
			this.vanishPoint = vanishPoint || {x:0,y:0};
			this.elements = [];
		}
		...
		Scene3D.prototype = {
			render:function (){
				...
				{
					this.transform3dTo2d (this.elements[i]);
					// TODO: do sorting, if need
				}
			},
			...
			add:function(element3D){
				...
				//Children.Add (sprite3D);
			},
			
			remove:function(element3D){
			},
				
			transform3dTo2d: function (element3D) {
				if (element3D.z <= -this.focalLength) 
				{
					if (element3D.visible) element3D.setVisible (false);
				} 
				else 
				{
					if (!element3D.visible) element3D.setVisible (true);
					
					var scale = this.focalLength / (this.focalLength + element3D.z);
					element3D.x2D = this.vanishPoint.x + element3D.x * scale;
					element3D.y2D = this.vanishPoint.y + element3D.y * scale;
			...
			to3D:function (x2D, y2D){
				
			}
		}
		
		function Box (element, x, y, width, height, dx, dy, density, circle)
		{
			...
			this.data = null;
			if (!circle)
				this.body = createBox2D (world, x + this.wHalf, y+this.hHalf, this.wHalf, this.hHalf, density || 1.0, {box:this});
			else
				this.body = createBall2D (world, x + this.wHalf, y+this.hHalf, this.wHalf, density || 1.0, {box:this});
		}
		
		Box.prototype = {
			update:function(){
				var element;
				this.x = (this.body.m_position0.x - this.wHalf) - this.dx;
				this.y = (this.body.m_position0.y - this.hHalf) - this.dy;
				var rot = this.body.m_rotation0 * 57.2957795;
				var style;
				
				if (this.data && this.data.pig)
				{
					//this.data.pig.currentAnimation.x = x; 
					//this.data.pig.currentAnimation.y = y;
					this.data.pig.currentAnimation.update();
					element = this.data.pig.currentAnimation.element;
					//log ("rot: "+ rot);
					var rotPos = rot < 0 ? -rot : rot;
					if (rotPos > 200)
					{
						killPig (this.data.pig);
						return;
					}
				}
				else
		...
		{
			this.img = img;
			//this.width = this.wInit = this.img.width / 20;
			this.width = this.wInit = this.img.width;
			this.height = this.hInit = this.img.height;
			this.wHalf = this.width * 0.5;
			this.hHalf = this.height * 0.5;
			this.x = 0;
			this.y = 0;
			...
			/*this.idleAnimation = new SpriteSheet (this.img, this.width, this.height);
		...
		Bird.prototype = {
			...
			setScale:function(value){
				this.scale = value;
				this.width = this.wInit * this.scale;
				this.height = this.hInit * this.scale;
				this.wHalf = this.width * 0.5;
				this.hHalf = this.height * 0.5;
				
				/*this.currentAnimation.scaleX = this.scale;
				this.currentAnimation.scaleY = this.scale;
				this.currentAnimation.width = this.width;
				this.currentAnimation.height = this.height;
				this.currentAnimation.spriteSheetWidth = this.img.width;
				this.currentAnimation.numFrames = Math.floor(this.currentAnimation.spriteSheetWidth/this.currentAnimation.width);
				this.currentAnimation.currentFrame = 0;
				this.currentAnimation.x = this.x2D;
				this.currentAnimation.y = this.y2D;
				this.currentAnimation.update();*/
			}
		}
		
		function Pig (img, x, y)
		{
			this.img = img;
			this.element = document.createElement("div");
			this.element.style.background = 'url('+this.img.src+')'; 
			this.element.style.width = img.width;
			this.element.style.height = img.height;
			this.element.style.position = "absolute";
			this.element.style.display = "block";
			...
			this.element.style.zIndex = 9999 + boxes.length;
			document.body.appendChild (this.element);
			//log ("this.img: ", this.element);
			
			this.x = x;
			this.y = y;
			this.width = this.img.width / 20;
			this.height = this.img.height;
			this.wHalf = this.width * 0.5;
			this.hHalf = this.height * 0.5;
			this.rot = 0;
			
			this.idleAnimation = new SpriteSheet (this.img, this.width, this.height, this.element);
			
			this.currentAnimation = this.idleAnimation;
			
			//createBox (element, density, circle, width, height)
			this.box = createBox (this.element, 0.1, true, this.width, this.height);
			this.box.data = {pig:this};
		}
		
		Pig.prototype = {
			update:function(){
				this.x = (this.body.m_position0.x);
				this.y = (this.body.m_position0.y);
				this.rot = this.body.m_rotation0;
				//var style = 'rotate(' + (this.body.m_rotation0 * 57.2957795) + 'deg)';
			}
		}
		
		// FUNCTIONS		
		function getElement (x, y, minW, minH, maxW, maxH)
		{
			var element = document.elementFromPoint(x, y);
			if (!element) return false;
			
			if (element["angryType"] && boxesDict[element.angryType]) return element;
			
			element = getTrueElement (element);
			
			if (minW && minH && maxW && maxH)
			{
				var elementW = parseInt (getStyle(element, "width"));
				var elementH = parseInt (getStyle(element, "height"));
				if (elementW < minW || elementH < minH || elementW > maxW || elementH > maxH) return false;
			}
			
			//console.log ("getElement: ", element);
			if (hasAnimated(element) || (getStyle(element, 'visibility') == 'hidden')) return false;
			
			//log ("element: "+ element + " - tagName: "+ element.tagName + " - className: " + element.className);
			//log ("element is bird: "+ (element == currentBird.img));
			if (!element || !element.tagName || specialTags.indexOf(element.tagName) > -1 || (element['getElementsByTagName'] && element.getElementsByTagName('*').length > 6)) return false;
			return setAnimated(element);
		}
		...
		function setAnimated(element){
			...
			for(var x = 0; x < childs.length; x++){
				childs[x]['animated'] = true;
				
				if(VERBOSE)
					childs[x].style.border = '2px solid orange';
			}
			
			return element;
		}
		
		function unsetAnimated(element){
			element['animated'] = false;
			
			var childs = element.getElementsByTagName('*');
			
			for(var x = 0; x < childs.length; x++){
				childs[x]['animated'] = false;
			}
		...
		function hasAnimated(element){
			if(element['animated']) return true;
			
			var childs = element.getElementsByTagName('*');
			
			for(var x = 0; x < childs.length; x++){
				if(childs[x] && childs[x]['animated']) return true;
			}
		...
		function update ()
		{
			...
			//simpleTween.updateTweens();
		}
		
		function collideWithDOM (bird)
		{
			setGameVisible (false);
			//log ("collideWithDOM");
			
			stopCaptureBirdTrail ();
			
			if (VERBOSE)
			{
				debugObjs.push ({x:bird.x2D-10, y:bird.y2D-10, w:20, h:20, c:"rgba(0,0,255,0.5)"});
			}
			
			var element = getElement (bird.x2D, bird.y2D);
			//console.log ("COLLIDE WITH: ", element);
			if (element)
			{
				hitElement (element, bird);
			}
			
			setImpact(bird, element);
			
			setGameVisible (true);
		}
		
		function hitElement (element, bird)
		{
			//console.log ("hitElement: ", element);
			var box;
			if (angryElements.indexOf (element) == -1)
			{
				log ("creating new box");
				box = createBox (element);
			}
			else
			{
				box = boxesDict[element.angryType];
				log ("existing element: "+element.angryType+ ", has box: " + box);
			}
			if (box != null && box.body != null)
			{
				addScore (100);
				log ("	applying impulse, has joint: " + (box.data && box.data.joint));
				if (box.data && box.data.joint)
				{
					world.DestroyJoint (box.data.joint);
					box.data = null;
				}
				box.body.ApplyImpulse (new b2Vec2(bird.vx*10000, bird.vy*10000), new b2Vec2(bird.x2D-wScrollX, bird.y2D-wScrollY));
				setTimeout (function(){
					killBox (box);
		...
		function setImpact (bird, fromElement)
		{
			var cols = 5;
			var rows = 5;
			var c;
			var r;
			var padding = 20;
			var areaW = (cols-1) * padding;
			var areaH = (rows-1) * padding;
			
			//log ("setImpact");
			
			if (VERBOSE)
				debugObjs.push ({x:bird.x2D - areaW/2, y:bird.y2D - areaH/2, w:areaW, h:areaH, c:"rgba(255,255,0,0.3)"});
			
			for (c = 0; c < cols; c++)
			{
				for (r = 0; r < rows; r++)
				{
					var x = bird.x2D - areaW/2 + c * padding;
					var y = bird.y2D - areaH/2 + r * padding;
					
					var element = getElement (x, y);
					if (element && element != fromElement && angryElements.indexOf (element) == -1)
					{
						if (VERBOSE) 
							debugObjs.push ({x:x-5, y:y-5, w:10, h:10, c:"rgba(0,255,0,0.8)"});
		...
		function updateDOM()
		{
			wScrollX = window.scrollX || document.documentElement.scrollLeft;
			wScrollY = window.scrollY || document.documentElement.scrollTop;
			
			worldAABB.minVertex.Set( - 200 + wScrollX, - 200  + wScrollY);
			worldAABB.maxVertex.Set( WIDTH + 200+ wScrollX, HEIGHT + 200 + wScrollY);
			
			world.m_gravity.x = gravity.x * 350;
			world.m_gravity.y = gravity.y * 350;
			...
			if (sheetAnimations && sheetAnimations.length > 0)
			{
				for (i = 0; i < sheetAnimations.length; i++)
				{
					var anim = sheetAnimations[i];
					anim.update();
				}
			}
			
			var offx = 17;
			var offy = document.body.scrollWidth > document.body.clientWidth > 0 ? 17 : 0;
			if (!isIE) offx = document.body.scrollHeight > document.body.clientHeight ? 17 : 0;
			
			var posScoreX = WIDTH - imgScore.width - offx + wScrollX;
			var posScoreY = HEIGHT - imgScore.height - offy + wScrollY;
			
			//score.style.left = posScoreX + "px";
			//score.style.top = posScoreY + "px";
		}
		...
		function checkCollisions()
		{
			...
			{
				bd1 = c.m_shape1.m_body;
				bd2 = c.m_shape2.m_body;
				
				// testing pigs to kill them
				if (bd1.m_userData && bd1.m_userData.box && bd1.m_userData.box.body && bd1.m_userData.box.data && bd1.m_userData.box.data.pig)
				{
					var speed = bd1.m_userData.box.body.m_angularVelocity;
					//console.log ("speed: ", speed);
					if (speed > 0.5)
					{
						killPig (bd1.m_userData.box.data.pig);
					}
				}
				
				// testing joints to remove them
				if (bd1.m_userData && bd1.m_userData.box && bd1.m_userData.box.data && bd1.m_userData.box.data.joint)
				{
					var speed = bd1.m_userData.box.data.joint.GetJointSpeed();
					speed = speed < 0 ? -speed : speed;
					if (speed > 1)
					{
						//log ("DESTROY JOINT!");
						world.DestroyJoint (bd1.m_userData.box.data.joint);
						bd1.m_userData.box.data = null;
					}
				}
			}
		}
		...
		function updateGame ()
		{
			...
			dx = forkPos.xtInit - ptMouse.x;
			dy = forkPos.ytInit - ptMouse.y;
			dist = Math.sqrt (dx * dx + dy * dy);
			angleMouse = Math.atan2 (dy, dx);
			radius = dist;
			if (angleMouse > angleMin && angleMouse < angleMax)
				radius = FORK_MIN_RADIUS;
			...
			{
				bird = birds[i];
				birdPosInit.x = forkPos.xtInit - scene3D.vanishPoint.x - bird.wHalf;
				birdPosInit.y = forkPos.ytInit - scene3D.vanishPoint.y - bird.hHalf;
				
				if (bird.isActive)
				{
					if (bird.isDragging)
					{
						bird.x = birdPosInit.x - Math.cos (angleMouse) * radius;
						bird.y = birdPosInit.y - Math.sin (angleMouse) * radius;
						//bird.z = -radius * 0.4;
						
						bird.vx = (birdPosInit.x-bird.x) * 0.4;
						bird.vy = (birdPosInit.y-bird.y) * 0.4;
						bird.vz = 3;
					}
					else
					{
						...
						if (bird.z > 100)
						{
							...
							bird.vy += Math.random() * -5 - 5;
							bird.vr += Math.random() * 0.4 - 0.2;
							collideWithDOM(bird);
						}
						
						if (bird.y2D > HEIGHT + 100)
						{
							killBird(bird);
							t = birds.length;
						}
			...
			if (currentBird)
			{
				if (currentBird.isActive)
					if (currentBird.isDragging)
					{
						forkPos.xt = currentBird.x2D + currentBird.wHalf - Math.cos (angleMouse) * 15;
						forkPos.yt = currentBird.y2D + currentBird.hHalf - Math.sin (angleMouse) * 15;
					}
					else
					{
						forkPos.xt += ((forkPos.xtInit - 10) - forkPos.xt) * 0.3;
						forkPos.yt += ((forkPos.ytInit+14) - forkPos.yt) * 0.3;
						
					}
				else
				{
					forkPos.xt = forkPos.xtInit - 10;
					forkPos.yt = forkPos.ytInit + 14;
					currentBird.x = birdPosInit.x;
					currentBird.y = birdPosInit.y;
				}
			}
		}
		...
		function killBird (bird)
		{
			...
			if (index > -1)
			{
				//log ("total birds before: "+ birds.length);
				//log ("bird killed: "+ bird + "("+index+")");
				birds.splice (index, 1);
				if (bird == currentBird) currentBird = null;
				bird = null;
			}
			log ("total birds: "+ birds.length);
		}
		
		function killPig (pig)
		{
			log ("killPig");
			var index = pigs.indexOf (pig);
			if (index > -1)
			{
				var explodeAnimation = new SpriteSheet (imgPigExplosion, imgPigExplosion.width/14, imgPigExplosion.height, null, true);
				explodeAnimation.x = pig.box.x - pig.box.wHalf;
				explodeAnimation.y = pig.box.y - pig.box.hHalf;
				explodeAnimation.looping = false;
				explodeAnimation.onComplete = function (){
					log ("hide explosion");
					var index = sheetAnimations.indexOf (explodeAnimation);
					if (index > -1) sheetAnimations.splice (explodeAnimation, index);
					explodeAnimation.element.style.display = "none";
					explodeAnimation = null;
				}
				sheetAnimations.push (explodeAnimation);
		...
		function killBox (box, hideElement)
		{
			//console.log ("killBox: ", box);
			world.DestroyBody(box.body);
			box.body = null;
			
			if (!hideElement) box.element.style.display = "none";
		...
		function draw ()
		{
			...
			// draw
			tthis.context.clearRect (0, 0, WIDTH, HEIGHT);
			
			// phone
			tthis.context.drawImage (imgPhone, phonePos.x-11.8, phonePos.y+66.35);
			// phone screen
			tthis.context.drawImage (imgPhoneScreen, phonePos.x+56, phonePos.y+78);
			
			// fork bottom
			tthis.context.drawImage (imgForkBottom, forkPos.x1-8.4, forkPos.y1-23);
			// grass
			tthis.context.drawImage (imgGrass, phonePos.x+92, phonePos.y+88);
			// blue bird
			tthis.context.drawImage (imgBlueBird, phonePos.x+154, phonePos.y+60);
			
			var strokeWidth = 6 - radius * 0.03;
			// fork rope 1
			tthis.context.beginPath ();
			tthis.context.strokeStyle = "#20190F";
			tthis.context.lineWidth = strokeWidth;
			tthis.context.lineCap = "round";
			tthis.context.lineJoin = "round";
			tthis.context.moveTo (forkPos.x2, forkPos.y2);
			tthis.context.lineTo (forkPos.xt, forkPos.yt);
			tthis.context.closePath ();
			tthis.context.stroke();
			
			if (currentTrail)
			{
				for (i = 0; i < currentTrail.length; i++)
				{
					tthis.context.beginPath ();
					tthis.context.lineWidth = 1;
					tthis.context.strokeStyle = "#555555";
					tthis.context.fillStyle = "#FFFFFF";
					tthis.context.arc (currentTrail[i].x, currentTrail[i].y, currentTrail[i].r, 0, TWO_PI, true);
					tthis.context.stroke();
					tthis.context.fill();
				}
			}
			...
			for (i = 0; i < birds.length; i++)
			{
				bird = birds[i];
				tthis.context.save();
				tthis.context.translate(bird.x2D+bird.wHalf, bird.y2D+bird.hHalf);
				tthis.context.rotate(bird.rot);
				tthis.context.drawImage (bird.img, -bird.wHalf, -bird.hHalf, bird.width, bird.height);
				//log ("bird.currentAnimation.sx: " + bird.currentAnimation.sx);
				//tthis.context.drawImage (bird.img, bird.currentAnimation.sx, 0, bird.width, bird.height, bird.x2D, bird.y2D, bird.width, bird.height);
				//tthis.context.drawImage (bird.img, 0, 0, bird.width, bird.height, bird.x2D, bird.y2D, bird.width, bird.height);
				tthis.context.restore();
			}
			...
			tthis.context.lineCap = "round";
			tthis.context.lineJoin = "round";
			tthis.context.moveTo (forkPos.x1, forkPos.y1);
			tthis.context.lineTo (forkPos.xt, forkPos.yt);
			tthis.context.closePath ();
			tthis.context.stroke();
			...
			// fork support
			tthis.context.save();
			tthis.context.translate (forkPos.xt, forkPos.yt);
			var rot = currentBird && currentBird.isActive && currentBird.isDragging ? angleMouse : -45;
			tthis.context.rotate (rot);
			tthis.context.drawImage (imgForkSupport, -imgForkSupport.width * 0.5, -imgForkSupport.height * 0.5)
			tthis.context.restore();
				
			// fork up
			tthis.context.drawImage (imgForkUp, forkPos.x1-8.4, forkPos.y1-23);
			
			// yellow bird
			tthis.context.drawImage (imgYellowBird, phonePos.x+101, phonePos.y+66);
			// red bird big
			tthis.context.drawImage (imgRedBirdBig, phonePos.x+205, phonePos.y+37);
			
			// debug
			if (VERBOSE)
			{
				drawWorld(world, tthis.context);
				if ( debugObjs.length > 0)
				{
					for (i = 0; i < debugObjs.length; i++)
					{
						var debugObj = debugObjs[i];
						tthis.context.fillStyle = debugObj.c || "rgba(255,0,0,0.3)";
						tthis.context.fillRect (debugObj.x, debugObj.y, debugObj.w || 5, debugObj.h || 5);
					}
				}
			}
		}
		
		function createBox (element, density, circle, width, height)
		{
			log ("createBox");
			
			setAbsolute (element);
			
			var w = width || parseInt(getStyle(element,'width'));
			var h = height || parseInt(getStyle(element,'height'));
			var x = parseInt(element.offsetLeft || getStyle(element, 'left'));
			var y = parseInt(element.offsetTop || getStyle(element, 'top'));
			
			var r = findPos(element);
			var rx = r[0];
			var ry = r[1];
			
			element.angryType = "ANGRYBIRDS_BOX"+boxes.length;
			var box = new Box (element, rx, ry, w, h, rx - x, ry - y, density || 1.0, circle || false);
			boxes.push (box);
			boxesDict[element.angryType] = box;
			angryElements.push (element);
			
			//if (VERBOSE)
			//{
				//debugObjs.push ({x:x, y:y, w:w, h:h, c:"rgba(255,0,0,0.1)"});
			//}
		...
		function getStyle(el,styleProp)
		{
			...
			try{
				...
				if (el.currentStyle && typeof el.currentStyle != 'undefined' ) {
					y = el.currentStyle;
				} else {
					y = document.defaultView.getComputedStyle(el, null);
				}

				y = styleProp ? y[styleProp] : y;
			
				if(styleProp && (styleProp == 'width' || styleProp == 'height')){
					y = parseInt(y);
					if(!isNaN(y) && y > 0){
						y = y + 'px';
					}
					else{
						var pp = toCamel(styleProp);
						var cY = el['client' + pp];
						var oY = el['offset' + pp];
						y = (cY || oY) + 'px';
					}
		...
		function setAbsolute(tg){
			var bk = document.createElement("ANGRYBIRDS");
			var cl = tg.cloneNode(true);
			
			if(tg.tagName.toUpperCase() == "ANGRYBIRDS") return;
			
			
			var w = getStyle(tg,'width'), 
				h = getStyle(tg,'height');
			
			bk.style.visibility = 'hidden';				
			bk.appendChild(cl);
			
			tg.style.width = (w == '' || w == '0px') ? 'auto' : w;
			tg.style.height = (h == '' || h == '0px') ? 'auto' : h;
			tg.style.top = tg.offsetTop + "px";
			tg.style.left = tg.offsetLeft + "px";
			tg.style.right = "auto";
		...
		function divideImage(img){
			...
			var fkimgarea = document.createElement('angryimgarea');
				...
				var pl = Math.floor(h / l);
				var rpl = h % l;
				var pc = Math.floor(w / c);
				var rpc = h % l;
				...
				while(cl < l){
					...
					while(acc < c){
						var peace = document.createElement('angryimgpeace');
							peace.style.display = 'block';
							peace.style.position = 'absolute';
							peace.style.width = pc + (acc == c-1 ? rpc : 0) + 'px';
							peace.style.height = pl + (cl == l-1 ? rpl : 0) + 'px';
							peace.style.background = 'url(' + img.src + ') no-repeat ';
							peace.style.backgroundPosition = '-' + (acc * pc) +'px -' + (cl * pl) + 'px';
							peace.style.top = (cl * pl) + 'px';
							peace.style.left = (acc * pc) + 'px';
							
						fkimgarea.appendChild(peace);
						acc++;
					}
					cl++;
				}
				
				fkimg.appendChild(fkimgarea);
				img.parentNode.replaceChild(fkimg, img);
		};
		...
		function getTrueElement(element)
		{
			var parent = element['parentNode'];

			if(parent){
				if(!(getStyle(parent,'overflow') != 'hidden') && parent.childNodes.length < 3)
					parent.style.overflow = 'visible';
				
				getTrueElement(parent);
			};
		...
		{
			tthis.gameDiv.style.visibility = visible ? "visible" : "hidden";
		}
		...
		function createPigs ()
		{
			...
			var rows = 6;
			var random = 50;
			var random2 = random*2;
			var marginLeft = 200+random;
			var marginRight = 200-random;
			var marginTop = 20+random;
			var marginBottom = 150-random;
			var padx = (WIDTH - (marginRight)) / cols;
			var pady = (HEIGHT - (marginBottom)) / rows;
			
			for (var c = 0; c < cols; c++)
			{
				for (var r = 0; r < rows; r++)
				{
					var x = marginLeft + c * padx + (Math.random()* random2 - random);
					var y = marginTop + r * pady + (Math.random()* random2 - random);
					if (VERBOSE)
					{
						debugObjs.push ({x:x, y:y, w:10, h:10, c:"rgba(255,0,255,0.7)"});
					}
					var element = getElement (x,y, 60, 20, 700, 200);
					if (element && angryElements.indexOf (element) == -1)
					{	
						var box = createBox (element);
						suspendBox (box);
						var pig = new Pig (imgPig, box.x + (box.width-42)*0.5, box.y-42);
						pigs.push (pig);
					}
				}
			}
			log ("totalPigs: "+ pigs.length);
			setGameVisible (true);
		}
		
		
		function suspendBox (box)
		{
			log ("suspendBox");
			var jointDef = new b2RevoluteJointDef();
			jointDef.anchorPoint.Set(box.x+box.width*0.5, box.y-10);
			jointDef.body1 = box.body;
			jointDef.body2 = world.GetGroundBody();
			box.data = {joint:world.CreateJoint(jointDef)};
		}
		
		function initCanvas ()
		{
			this.canvas = document.createElement('canvas');
			this.canvas.angryType = "ANGRYBIRDS";
			...
			addEvent(this.canvas, 'mousedown', canvasMouseDown);
			addEvent(this.canvas, 'mouseup', canvasMouseUp);
			addEvent(this.canvas, 'mousemove', canvasMouseMove);
		}
		...
		function addBird()
		{
			currentBird = currentAngry = new Bird (imgRedBird);
			resetBird ();
			scene3D.add (currentBird);
		...
		{
			//divideImages();
			//setInterval(divideImages, 10000);
			
			tthis = this;
			this.gameDiv = document.createElement ('ANGRYBIRDS');
			this.gameDiv.angryType = "ANGRYBIRDS";
		...
		function initGame ()
		{
			...
			if (VERBOSE) debugObjs.push (scene3D.vanishPoint);
		...
		{
			// score
			score = document.createElement('ANGRYBIRDS');
			score.angryType = "ANGRYBIRDS_SCORE";
			score.style.width = imgScore.width + "px";
			score.style.height = imgScore.height + "px";
			score.style.position = "fixed";
			score.style.bottom = "0px";
			score.style.right = "0px";
			
			score.style.zIndex = 10001;
			score.style.background = 'url('+imgScore.src+')'; 
			score.style.textAlign = 'center';
			document.body.appendChild (score);
			
			// points
			scorePoints = document.createElement ('ANGRYBIRDS');
			scorePoints.angryType = "ANGRYBIRDS_SCORE";
			scorePoints.style.position = "relative";
			scorePoints.style.left = "5px";
			scorePoints.style.top = "115px";
			scorePoints.style.font = "35px sans-serif";
			scorePoints.style.color = '#ffffff';
			score.appendChild (scorePoints);
			addScore (0);
			
			// share buttons
			twitter = document.createElement('div');
			twitter.angryType = "ANGRYBIRDS_SCORE";
			twitter.style.position = "relative";
			twitter.style.left = "40px";
			twitter.style.top = "115px";
			twitter.style.width = imgTwitter.width + "px";
			twitter.style.height = imgTwitter.height + "px";
			twitter.style.background = 'url('+imgTwitter.src+')'; 
			twitter.style.cursor = "pointer"
			addEvent (twitter, "mouseover", function(){twitter.style.cursor = "hand"});
			addEvent (twitter, "mouseout", function(){twitter.style.cursor = "pointer"});
			addEvent (twitter, "click", function(){
				//console.log ("click twitter");
				twitterPost (TWITTER_TITLE.replace ("[score]", totalScore).replace("[url]", SHARE_URL));
			});
			score.appendChild (twitter);
			
			facebook = document.createElement('div');
			facebook.angryType = "ANGRYBIRDS_SCORE";
			facebook.style.position = "relative";
			facebook.style.left = "71px";
			facebook.style.top = "92px";
			facebook.style.width = imgFacebook.width + "px";
			facebook.style.height = imgFacebook.height + "px";
			facebook.style.background = 'url('+imgFacebook.src+')'; 
			facebook.style.cursor = "pointer";
			addEvent (facebook, "mouseover", function(){facebook.style.cursor = "hand"});
			addEvent (facebook, "mouseout", function(){facebook.style.cursor = "pointer"});
			addEvent (facebook, "click", function(){
				//console.log ("click facebook, score: " + totalScore);
				facebookPost (SHARE_URL, FACEBOOK_TITLE, "", FACEBOOK_DESC.replace ("[score]", totalScore));
			});
			score.appendChild (facebook);
		}
		...
		function addScore (points)
		{
			...
			scorePoints.innerHTML = "" + totalScore + "";
			//console.log ("addScore html: " + scorePoints.innerHTML);
			//console.log ("	this: " + this);
			//console.log ("	scorePoints: " + scorePoints);
		}
		
		function createBox2D(world, x, y, width, height, density, data)
		{
		...
		function createBall2D(world, x, y, radius, density, data)
		{	
			if (!density) density = 1.0;
		...
		function preloadImgs ()
		{
			...
			for (var i =0 ; i < total; i++)
			{
				...
				imgs[i].onload = function (){
					...
					{
						imgRedBird = imgs[0];
						imgRedBirdBig = imgs[1];
						imgYellowBird = imgs[2];
						imgBlueBird = imgs[3];
						imgGrass = imgs[4];
						imgPhone = imgs[5];
						imgPhoneScreen = imgs[6];
						imgForkUp = imgs[7];
						imgForkBottom = imgs[8];
						imgForkSupport = imgs[9];
						imgPig = imgs[10];
						imgScore = imgs[11];
						imgPigExplosion = imgs[12];
						imgTwitter = imgs[13];
						imgFacebook = imgs[14];
						initGame();
					}
		...
		function onResize ()
		{
			...
			tthis.canvas.setAttribute('width', WIDTH);
			tthis.canvas.setAttribute('height', HEIGHT);
			this.gameDiv.style.width = WIDTH;
			this.gameDiv.style.height = HEIGHT;
			
			scene3D.vanishPoint.x = WIDTH * 0.5;
			scene3D.vanishPoint.y = HEIGHT * 0.5;
			
			phonePos.x = WIDTH / 4;
			phonePos.y = HEIGHT - 206 - 25;
			
			forkPos.x1 = phonePos.x + 94;
			forkPos.y1 = phonePos.y + 22;
			forkPos.x2 = phonePos.x + 121;
			forkPos.y2 = phonePos.y + 24;
			forkPos.xtInit = phonePos.x + 108;
			forkPos.ytInit = phonePos.y + 23;
			
			PAGE_HEIGHT = document.body.scrollHeight;
			
			if (currentBird)
			{
				birdPosInit.x = forkPos.xtInit - scene3D.vanishPoint.x - currentBird.wHalf;
				birdPosInit.y = forkPos.ytInit - scene3D.vanishPoint.y - currentBird.hHalf;
			}
		}
		...
		function canvasMouseUp (e)
		{
			...
			if (currentBird.isDragging) drop (currentBird)
		}
		...
		function canvasMouseDown (e)
		{
			log ("canvasMouseDown");
			e = e || window.event;
			ptMouse.x = e.pageX - wScrollX;
			ptMouse.y = e.pageY - wScrollY;
			
			if (checkHit (currentBird.x2D, currentBird.y2D, currentBird.width, currentBird.height, ptMouse.x, ptMouse.y)) drag (currentBird);
		}
		
		function canvasMouseMove (e)
		{
			e = e || window.event;
			// fazer hack para outros browsers
			ptMouse.x = e.pageX - wScrollX;
			ptMouse.y = e.pageY - wScrollY;
		}
		...
		function drag (bird)
		{
			dragOffset.x = ptMouse.x - bird.x2D;
			dragOffset.y = ptMouse.y - bird.y2D;
			bird.isDragging = true;
			if (!bird.isActive) bird.isActive = true;
		}
		
		function drop (bird)
		{
			bird.isDragging = false;
			currentMovingBird = bird;
			captureBirdTrail ();
			setTimeout (function(){
				addBird ();
			}, 1000);
		...
		function captureBirdTrail ()
		{
			currentTrail = [];
			intervalTrail = setInterval(function(){
				//log ("get bird trail");
				currentTrail.push ({x:currentMovingBird.x2D+currentMovingBird.wHalf, y:currentMovingBird.y2D+currentMovingBird.hHalf, r:(Math.random()*2+1.5)});
			}, 25)
		}
		
		function stopCaptureBirdTrail ()
		{
			clearInterval (intervalTrail);
			currentMovingBird = null;
		}
		
		function checkHit (x1, y1, w, h, x2, y2)
		{
		...
		function setWalls()
		{
			...
			//walls[0] = createBox2D(world, WIDTH / 2 + wScrollX, -wall_thickness + wScrollY, WIDTH, wall_thickness, true);
			walls[0] = createBox2D(world, WIDTH / 2 + wScrollX, HEIGHT + wall_thickness + wScrollY, WIDTH, wall_thickness, true);
			walls[1] = createBox2D(world, -wall_thickness + wScrollX, HEIGHT / 2 + wScrollY, wall_thickness, HEIGHT, true);
			walls[2] = createBox2D(world, WIDTH + wall_thickness + wScrollX, HEIGHT / 2 + wScrollY, wall_thickness, HEIGHT, true);
		...
		function facebookPost(url, title, imageSrc, summary)
		{
			t=encodeURIComponent(title);
			d=encodeURIComponent(summary);
			u=encodeURIComponent(url);
			i=encodeURIComponent(imageSrc);
			share_url='http://www.facebook.com/sharer.php';
			share_url+='?s=100&p[title]='+t+'&p[summary]='+d+'&p[url]='+u;
			share_url+='&p[images][0]='+i;
			share_url+='&t='+t+'&e='+d;
			var myWindow = window.open(share_url, '', 'width=800,height=500');
		}
		
		function twitterPost (message)
		{
		...
		var sheetAnimations = [];
		var currentBird;
		var currentMovingBird;
		var currentTrail;
		var intervalTrail;
		var score;
		var scorePoints;
		var totalScore = 0;
		var specialTags = ['HTML', 'HEAD', 'BODY', 'SCRIPT', 'TITLE', 'META', 'STYLE', 'LINK', 'TD', 'TR', 'TH', 'THEAD', 'TBODY', 'ANGRYBIRDS'];
		if ( window.ActiveXObject )
			specialTags = ['HTML', 'HEAD', 'BODY', 'SCRIPT', 'TITLE', 'META', 'STYLE', 'LINK', 'TD', 'TR', 'TH', 'THEAD', 'TBODY', 'ANGRYBIRDS', 'SHAPE', 'LINE', 'GROUP', 'IMAGE', 'STROKE', 'FILL', 'SKEW', 'PATH', 'TEXTPATH', 'INS'];
		...
		var angleMouse;
		var angleMin = -111 * Math.PI / 180;
		var angleMax = -71 * Math.PI / 180;
		var radius=0;
		var MAX_RADIUS = 80;
		var FORK_MIN_RADIUS = 17;
		...
		// positions
		var phonePos = {x:0, y:0};
		var forkPos = {x1:0, y1:0, x2:0, y2:0, xt:0, yt:0, xtInit:0, ytInit:0};
		var birdPosInit = {x:0, y:0, z:0};
		
		var wScrollX=0;
		var wScrollY=0;
		
		// imgs - inited as string, after they become the image itself
		var imgRedBird = "red_bird.png";
		var imgRedBirdBig = "red_bird_big.png";
		var imgYellowBird = "yellow_bird.png";
		var imgBlueBird = "blue_bird.png";
		var imgGrass = "grass.png";
		var imgPhone = "phone.png";
		var imgPhoneScreen = "phone_screen.png";
		var imgForkUp = "fork_up.png";
		var imgForkBottom = "fork_bottom.png";
		var imgForkSupport = "fork_support.png";
		var imgPig = "pig.png";
		var imgScore = "score.png";
		var imgPigExplosion = "pig_explosion.png";
		var imgTwitter = "icon_twitter.png";
		var imgFacebook = "icon_facebook.png";
		var imgs = [imgRedBird, imgRedBirdBig, imgYellowBird, imgBlueBird, imgGrass, imgPhone, imgPhoneScreen, imgForkUp, imgForkBottom, imgForkSupport, imgPig, imgScore, imgPigExplosion, imgTwitter, imgFacebook];
		
		init ();
    }
	
	/**
	 * SimpleTween - very very simple tween engine, focused on animate any numerical property of any object
	 * @author - André Mattos - www.ma77os.com
	 * TODO: 	- implementar intervalo interno, 
	 * 			- limpeza de tweenObjects quando os tweens acabarem
	 * 			- Acertar time real do tempo (getTimer)
	 * 			- no tweenAttribute, tratar o valor "px" para propriedades x, y, width, height...
	 */
	...
	SimpleTween = function()
	{
		...
		this.tween = function (target, time, props, delay, onComplete)
		{
			var scope = this;
			...
			{
				var start = target[p];
				if (!isNaN(props[p])) propsList.push ({prop:p, start:start, change:props[p] - start});
			}
			var tweenObj = {target:target, props:propsList, duration:time, onComplete:onComplete};
			setTimeout (function(){
				tweenObj.time = scope.time;
				scope.tweenObjects.push (tweenObj);
			},delay != null ? delay * 1000 : 0);
			return tweenObj;
		}
		
		this.tweenAttribute = function (target, time, props, delay, onComplete)
		{
			var scope = this;
			props = props != null ? props : {};
			var propsList = [];
			for (var p in props)
			{
				var start = parseFloat(target.getAttribute(p));
				if (!isNaN(props[p])) propsList.push ({prop:p, start: start, change:props[p] - start});
			}
			var tweenObj = {target:target, props:propsList, duration:time, onComplete:onComplete};
			tweenObj.isAttribute = true;
			setTimeout (function(){
				tweenObj.time = scope.time;
				scope.tweenObjects.push (tweenObj);
			},delay != null ? delay * 1000 : 0);
		...
		{
			if (this.tweenObjects.length == 0) return;
			//log ("updateTweens" + this.time);
			this.time+=0.04;
			var i=0;
			var j=0;
			var tweenObj;
			var tweenTime;
			for (i; i<this.tweenObjects.length; i++)
			{
				if (this.tweenObjects[i] == null) continue;
				tweenObj = this.tweenObjects[i];
				tweenTime = this.time-tweenObj.time;
				if (tweenTime <= tweenObj.duration)
				{
					for (j=0; j < tweenObj.props.length; j++)
					{
						var prop = tweenObj.props[j].prop;
						var start = tweenObj.props[j].start;
						var change = tweenObj.props[j].change;
						var value = this.easeOutQuad (tweenTime, start, change, tweenObj.duration);
						if (!tweenObj.isAttribute)
						{
							tweenObj.target[prop] = value + "px";
						}
						else
						{
							tweenObj.target.setAttribute(prop, value);
						}
					}
				}
				else
				{
					if (tweenObj.onComplete != null) tweenObj.onComplete();
	...
	var simpleTween = new SimpleTween();
	simpleTween.init();
	...
	if ( window.ActiveXObject && ! document.createElement('canvas').getContext ) {
		...
		script.src = JS_URL + 'excanvas.js';
		document.getElementsByTagName('head')[0].appendChild(script);
	}
	else angryBirds = new AngryBirds();
